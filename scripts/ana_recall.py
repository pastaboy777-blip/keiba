...
# 叩き3戦目以降と重なると"実際に上乗せする"相性タグ(§20-C・南関1053R切り分け済)。
# X&叩き3+ の複勝率 − X&非3+ の複勝率 が +3%超のものを採用(=叩き3+が真に効く土台)。
# 除外：上がりP上位(-0.1%)/差し(+1.0%)/グリップ穴(+1.3%)/持続血統(+1.9%)/小型(+2.1%)=タグ単体の力。
_TATAKI_SYNERGY = {"持続巡航", "斤量減", "上がりP×短縮", "距離短縮",
                   "差し想定", "血統湿", "強差し", "道悪巧者"}
...
def edges_for(e, today_dist, small_bias=True, pace=None, agari_pct=None, today=None,
    ...
    if today is not None:
        ...
        if isinstance(_tn, int) and _tn >= 3:
            if today_dist and 1300 <= today_dist <= 1900:
                tags.add("叩き3戦目穴")
            # 叩き3+×相性タグ(§20-C切り分け済)＝"Xの中で叩き3+が実際に上乗せした"組合せのみ採用。
            # これら相性タグは叩き3+と重なると複勝14-20%(距離短縮/斤量減/上P×短縮は単体0.9→重ね1.3に跳ねる)。
            # 上乗せ無(上がりP上位/差し/グリップ穴/持続血統/小型)は除外。距離帯は問わない(全帯で上乗せ確認)。
            if tags & _TATAKI_SYNERGY:
                tags.add("叩き3相性穴")
    return tags, cw
