...
# 高知ファイナル専用(learn_zubu_kochi.pyで再学習・24ヶ月215件で確定)。頑健な構造は
# 「ability(地力)≈0=C級で実績が当てにならない・jockeyが支配的」。※12ヶ月時の+4pt改善は
# 倍データで消失(=小標本ノイズ)。デフォルト比の明確なエッジは未確認だが、実績を過信しない
# 意図でability抑制を残す。ファイナル(最終R)でのみ使う。実利は買い方(後方穴BOX)と穴騎手。
ZUBU_V2_WEIGHTS_KOCHI_FINAL = {"jockey": 0.36, "ability": -0.05, "senkou": 0.01, "agari": 0.08}
