...
消せない理由（1つでもあれば残す）:
    ...
    A2 当条件勝ち   同じ場・同じ距離・同じ馬場種別に**勝ち鞍がある**（走数は問わない）

    ★A の「2走以上」だけにすると、当条件1走1勝の馬が落ちる。
      2026-09-20 阪神10R で 2.8倍の1番人気ソルチェリア（阪神ダ1800を1走1勝）を
      「消す」側に置いた。オールカマーでレガレイラを落としたのと同じ原因。
      **少ない走数の勝ち鞍は、率で測ると必ず消える。別条件として立てる。**
...
def leg(rid: str, base, place_hint: str | None = None, force: bool = False):
    ...
    for r in rec:
        ...
        if id(r) in topa:
            ...
            why.append(f"A2当条件{r['cond'][1]}勝({r['cond'][0]}走)")
        if id(r) in topz:
