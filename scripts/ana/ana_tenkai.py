...
前後半（川崎84R＝2026/7/6-7/10, 7/27, 7/28 で検証）:
  配当は全7日で安定して後半が高い。
    三連単 中央値  前半(1-6R) 5,150円  →  後半(7-12R) 14,740円
    1万円超の割合  31%(13/42)          →  60%(25/42)
  → 勝負を厚くするなら7R以降。これは事前に使ってよい。

  一方「後半ほどハナが強くなる」は日替わりで、事前には決められない。
    先頭の倍率 前半→後半：7/6 2.07→1.13 / 7/7 3.33→1.13 / 7/8 2.96→1.19
                          7/9 1.74→2.48 / 7/10 2.08→1.56 / 7/27 1.81→2.87 / 7/28 1.89→3.15
    5日が下降・2日が上昇。84Rの平均では前半2.25 → 後半1.91 でむしろ前半のほうがハナ有利。
  → 位置バイアスの前後半差は固定値にせず、6R終了後に `rachi.py` の
     「前後半の向き（当日実測）」で判定してから後半の狙いを決める。
...
HALF_NOTE = {
    "前半": "前半(1R〜6R)：三連単の中央値5,150円・1万超31%。堅めに収まりやすい時間帯。",
    "後半": "後半(7R〜12R)：三連単の中央値14,740円・1万超60%。勝負を厚くするならここ。",
}


def half_of(rid):
    """楽天RACEIDの末尾2桁＝R番号から前半/後半を返す。"""
    try: rr = int(str(rid)[-2:])
    except Exception: return None
    return "前半" if rr <= 6 else "後半"
