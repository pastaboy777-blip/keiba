...
きっかけ（2026-08-02 船橋、本人の指摘「刺激がある馬が穴？」）:
  全110頭を「乗り替わり・距離変更・競馬場替わり」の数で割ると、
      変化ゼロ 32頭 → 0勝・単0%
      変化1つ  35頭 → 4勝・単56.6%
      変化2つ  26頭 → 5勝・単154.6%
      変化3つ  17頭 → 3勝・単314.1%
  ...
  ★ここで数えるのは「競馬場替わり」であって転入ではない。浦和・川崎・大井は同じ南関の
    別会場なので、そこを使って戻っても所属は変わらない。所属が動くのは佐賀・金沢・
    名古屋・JRAからの馬だけで、そちらは競馬場替わりに転入が重なった形になる。
  ★ただし1日ぶんである。しかも「競馬場替わり」だけを21,394走で測ったときは逆に出た
    （開催初日の競馬場替わり馬は勝率5.8%で、その日の全馬9.5%を下回った）。
...
def main():
    ...
    print("── 種類別 ──")
    line("競馬場替わり", [x for x in runs if x["tr"]])
