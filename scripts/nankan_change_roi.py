...
きっかけ（2026-08-02 船橋、本人の指摘「刺激がある馬が穴？」）:
  全110頭を「他場を使った・乗り替わり・距離変更」の数で割ると、
      変化ゼロ 32頭 → 0勝・単0%
      変化1つ  35頭 → 4勝・単56.6%
      変化2つ  26頭 → 5勝・単154.6%
      変化3つ  17頭 → 3勝・単314.1%
  ...
  ★浦和・川崎・大井は同じ南関の別会場なので、そこを使って戻るのは転入ではなく遠征である。
    転入と呼べるのは佐賀・金沢・名古屋・JRAからの馬だけ。ここでは「他場を使った」で数える。
  ★ただし1日ぶんである。しかも「他場帰り」だけを21,394走で測ったときは逆に出た
    （開催初日に他場を使ってきた馬は勝率5.8%で、その日の全馬9.5%を下回った）。
...
def main():
    ...
    print("── 種類別 ──")
    line("他場を使った", [x for x in runs if x["tr"]])
