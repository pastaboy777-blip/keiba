...
補正の実装状況（詳細は `core/bt.py` の冒頭）:
   基準タイム・馬場比・斤量・年齢・素点   … 有効
   **馬場状態補正**                     … 有効。道悪の偏り −2.36点 → −0.00点
   ペース補正ブレンド                    … 実装したが**既定でオフ**（効果が無く悪化した）
   不利補正                            … **南関では作れない**（不利データが存在しない）
   コース形態係数                        … 不要（基準を場×距離で持っているので吸収済み）
   Cap / Floor                        … 未実装

⚠️ 係数（斤量の秒/kg、騎手ランク）は**まだ最適化していない初期値**。
   年齢補正と馬場状態補正は実測値に置き換えてある。
"""
...
def main() -> None:
    ...
    print("  ⚠️ ペース補正は実装したが**既定でオフ**。実測でペースの偏りを")
    print("     −1.09点 → −1.37点 に悪化させたため（core/bt.py の冒頭に詳細）。")
    print("     残る偏りは1点程度なので、順位の解釈を変えるほどではない。")
