...
PLACE_FRONT_PRIOR = {"大井": 3, "船橋": 1}
# 場別の穴脚質フラグ: True=穴は逃げ先行に偏る(差しはヒモ限定)/False=穴脚質フラット(差しも頭級で買える)。
# 大井=穴の75%が逃げ先行→差しはヒモ。船橋=穴フラット(前54%)・短距離後方44%→差しも頭で狙える。
PLACE_ANA_FRONT = {"大井": True, "船橋": False}
