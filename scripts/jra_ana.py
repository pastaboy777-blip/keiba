...
def ana_score(cond_rank, pop3, worst, field, cond, top2):
    ...
    ★★ 2026-07-30 検証の結果このスコアは却下された ★★
    6月8日分（別期間）で再現しない。上位40頭のとき：
        7月 20倍+的中11本・複勝回収182%  →  6月 2本・64%（現行スコアと同数・下回る）
    7月の11本は1,173頭から48通り以上を試して見つけた最良値で、過去に合わせただけ。
    「条件指数<実力-0.5」の15.8%も38頭中6本と母数が薄い。
    否定された仮説として残す（消すと同じ発想を再発明するため）。詳細＝
    notes/live/jra_ana_kensho.md
    """
...
def candidates(D, par, cond_rank=4, skip_rank=3, min_pop=6.0,
    ...
    戻り値の各行は rid / umaban を持つので、結果と機械的に突き合わせられる。
    """
    rows = []
    for rid, r in D.items():
        ...
        if len(hs) < 6:
            continue
        honmei = {id(x) for x in sorted(hs, key=lambda x: -x["top2"])[:skip_rank]}
        cand = sorted([x for x in hs if x["cond"] is not None], key=lambda x: -x["cond"])
        for rank, x in enumerate(cand, 1):
            if rank > cond_rank or id(x) in honmei:
                continue
            if not x["pop"] or x["pop"] < min_pop:
                continue
