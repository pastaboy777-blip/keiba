...
── ★印：4〜6番人気 × 前に行くと読める馬 ─────────────────
  ...
  **当てる対象を「着順」から「4角の位置」に変える**という案。持ち越しが
  一番強い変数（脚質 r=+0.507）だけを予想し、一番効く馬場は**その日の
  1〜4Rで測って**5R以降に使う（予想しない）。

  実測（大井8/12〜17＋川崎8/18 の 5R以降・過去2走以上取れた401頭）:

      予想4角位置        n     3着内   複勝回収  上位3件抜き
      前(pred≤0.30)   109   **42.2%**  83.9%    66.5%
      中               120     23.3%   53.1%    35.7%
      後(pred>0.55)    172     15.7%   76.3%    40.7%
      （全体 401頭 25.2% / 71.4% / 54.4%）

  **オッズ帯で割ると、効く場所が1か所に集中する:**

      1〜3人気  前 66.0%(回収91.7%) 対 後 50.0%(95.0%)  ← 織り込み済み
      **4〜6人気  前 42.3%(99.6%) 対 中 14.3%(28.8%)**   ← **同じ帯で3倍**
      7人気以下  前  6.5%(55.8%) 対 後  9.0%(79.8%)     ← 逆。能力が足りない

  → 1〜3人気は全員が見るので効率的。7人気以下は前に行けても勝てない。
     **4〜6人気だけが「勝てる能力はあるが誰も研究しない帯」。**

  ⚠️ 鍵のセルは **n=26**（11頭が3着内）。z≈2.6 だが**9セル見た中の1つ**なので
     たまたまの可能性は消せない。
  ⚠️ 複勝回収 99.6% は**上位3件を抜くと 71.3%**。まだ儲かる水準ではない。
  ⚠️⚠️ **7日全部が前有利だった（+0.36〜+0.71）。後ろ有利の日を1日も見ていない。**
     「馬場で切り替える」部分は**未検証**。逆バイアスの日が来たら必ず記録すること。
...
TREND_BAD = 0.35
#: ★印を出す帯。**4〜6番人気だけ、市場が脚質を織り込んでいない**（下の注記）。
STAR_POP = (4, 6)
#: 「前に行くと読める」正規化4角位置（過去3走平均）。
STAR_POS = 0.30
...
def main() -> None:
    ...
    for rno in range(1, 13):
        ...
        for i, x in enumerate(rows[:5]):
            star = ("★" if (x["pop"] and STAR_POP[0] <= x["pop"] <= STAR_POP[1]
                            and x["pos"] is not None and x["pos"] <= STAR_POS)
                    else " ")
            tmark = ("↑" if x["tr"] is not None and x["tr"] <= -TREND_BAD else
                     "↓" if x["tr"] is not None and x["tr"] >= TREND_BAD else " ")
            print(f"  {mark[i]}{star}{x['u']:>2} {x['name']:<12} {x['band']:<5}"
