...
## 楽天競馬の出馬表から予想する(前5走の馬柱つき・1枚で完結)

netkeiba は結果とオッズが別ページだが、**楽天競馬の出馬表ページは1枚に
前5走の馬柱(着順・場・距離・頭数・馬場・コーナー通過順位・上がり3F・人気)
まで含む**ため、これ1枚で脚質・末脚・出走間隔・叩きの評価に必要なデータが揃う。

```bash
# 川崎 2026-06-15 6R/7R/8R を予想(保存HTMLがあればオフライン、無ければ取得)
python3 scripts/predict_kawasaki_0615.py
```

- パーサ: `src/nankeiba/ingest/rakuten.py`(`parse_race_card(html) -> {meta, horses}`)。
  人気馬(`<span class="hot">`)・穴馬(`<span class="dark">`)でオッズが内側spanに
  入る点も処理する。
- RACEID(18桁)は `YYYYMMDD + 10桁`、末尾2桁がレース番号(`rakuten.race_id`)。
- 出力: 予想印(◎○▲△☆)・3着内率・脚質・**市場との乖離(妙味/危険)**・
  三連複/三連単の確率上位とフォーメーション。
- 注意: 三連複/三連単の実オッズはJS描画で静的取得できないため、期待値判定は
  単勝オッズ(人気)との乖離で代替している。実オッズが得られれば
  `core/betting.select_ev_bets` でEV判定に置き換えられる。
- **節度を持って利用すること**(アクセス間隔・利用規約・個人利用の範囲)。
